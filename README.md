...
- 🧑🤝🧑 **4 人设按品类自动匹配 + 人设豁免** — `references/persona-*.md` 四个完整人设（口吻 / 痛点 / 词汇表 / 常用 hook）。按产品品类自动匹配（学生项目→学生党、工具→转码人、框架/开源→技术博主、简历项目→求职者），用户可手动 override。人设豁免以 `assets/banned-words.json` 的 `persona_exemptions` 为唯一权威（如学生党的"绝了/宝藏/真的香"），`banned_words_check.py --persona` 自动拆分豁免命中与需改写命中；换人设则需重检
...
- 🎯 **防误报 + 最长匹配去重** — `banned_words_check.py` 内置四道防误报：单字违禁词组合跳过（"最"后接后/初/近/终/先按"最后"等中性词跳过；"冲"前接缓/后接突/洗/锋/刺/击按"缓冲/冲突"等跳过）；"第一"后接量词（个/次/天/年…）按序数事实陈述跳过；脚本/代理/辅助函数等高频中性技术词不入清单；同一段文字多条命中做最长匹配去重（"性能最好"只报"最好"一次）。解决"正常技术文案被误伤强改"的问题
- 🔢 **数字卖点必须有出处** — 性能/规模类数字只能来自 README、源码、实测或用户提供；无出处的删数字、改定性或标"(待实测)"，禁止编造——对一个"可直接发布"的 skill，编数字等于让用户发虚假宣传
...
└── tech-finds-deep-dive/              # skill 本体
    ...
    │   ├── banned_words_check.py      # 违禁词自检：扫描+命中报告+改写建议（防误报+最长匹配去重+--persona 豁免）
